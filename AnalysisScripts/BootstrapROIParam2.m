...
if computeBootstrap
    ...
    for r=1:7
        ...
        %compare affective vs cognitive
        tv=zeros(270);
        pain_inds=ceil(FullDataSet.Y/6)==1;
        cog_inds=ceil(FullDataSet.Y/6)==2;
        neg_inds=ceil(FullDataSet.Y/6)==3;
        tv(pain_inds,neg_inds)=1;
        tv(pain_inds,cog_inds)=-1;
        tv(cog_inds,neg_inds)=-1;
        tv(eye(270)==1)=nan;
        
        tv(tv>0)=1;%/length(find(tv>0));
        tv(tv<0)=-1/2;%length(find(tv<0));
        tv=tv';
        pX=[pX squareform(tril(tv,-1))'];
        
        %compare pain and emotion
        tv=zeros(270);
        pain_inds=ceil(FullDataSet.Y/6)==1;
        cog_inds=ceil(FullDataSet.Y/6)==2;
        neg_inds=ceil(FullDataSet.Y/6)==3;
    ...
    save([basedir  'Results' filesep 'bsModCompWithinROIParam2.mat'],'b', '-v7.3');
