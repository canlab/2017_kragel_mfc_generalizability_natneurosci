...
%         b_null=squeeze(b_null(:,r,:)); %bootstrap distribution of betas on a single iteration
        
        b_ste = squeeze(nanstd(b_null));
        b_mean = squeeze(nanmean(b_null));
        b_ste(b_ste == 0) = Inf;
        b_Z = b_mean ./ b_ste;
        b_P(it,:,:) = 2 * (1 - normcdf(abs(b_Z))); %p value for single iteration
        
       it/1000 
    end

    save([basedir 'Results' filesep 'false_positive_rate_simulation.mat'],'b_P','null_sample_betas')
